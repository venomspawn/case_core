  ...
  module EventProcessors
    ...
    module IssueProcessorSpecHelper
      ...
      def case_closed_at(c4s3)
        closed_at = case_attributes(c4s3.id).dig(:closed_at)
        closed_at && Time.parse(closed_at)
      end
      ...
      def case_issued_at(c4s3)
        issued_at = case_attributes(c4s3.id).dig(:issued_at)
        issued_at && Time.parse(issued_at)
      end
    end
  end
end
