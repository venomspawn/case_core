...
*   `CC_DB_POOL` — максимальное количество соединений с базой данных на каждый
    процесс, включая дочерние процессы Puma (по умолчанию принимает значение
    4).
*   `CC_DB_WAIT` — максимальное время ожидания соединения с базой данных в
    секундах (по умолчанию принимает значение 5).
...
### Настройка автоматического обновления библиотек

*   `CC_FETCHER_CRON` — cron-строка с периодом обновления (см. также описание
    ниже в секции «Загрузка бизнес-логики»).

### Настройка автоматического удаления записей бездокументных файлов

*   `CC_DFC_CRON` — cron-строка с периодом проверки на наличие записей файлов,
    на которые не ссылается ни одна запись документа.
*   `CC_DFC_AGE` — минимальная разница в секундах между текущим временем и
    временем создания записи файла, на который не ссылается ни одна запись
    документа, чтобы запись подлежала удалению.

### Настройка автоматического удаления записей атрибутов со значением `nil`

*   `CC_EAC_CRON` — cron-строка с периодом проверки на наличие записей
    атрибутов заявок и межведомственных запросов со значением `nil`.
...
## Автоматическое удаление записей бездокументных файлов

Приложение предлагает сервис автоматического удаления записей файлов, на
которые не ссылается ни одна запись документа. Сервис запускается отдельным
процессом вместе с основным приложением и с некоторой периодичностью проверяет
наличие искомых записей файлов. Сервис удаляет только те записи файлов, которые
были созданы достаточно давно (см. настройку минимальной разницы между текущим
временем и временем создания записи файла выше). При работе сервис выводит
сообщения в журнал событий о найденных записях и ходе удаления.
...
*   `bundle exec rake case_core:run_dfc` — запуск только сервиса
    автоматического удаления записей файлов, на которые не ссылается ни одна
    запись документа.

*   `bundle exec rake case_core:run_eac` — запуск только сервиса
    автоматического удаления записей атрибутов заявок и межведомственных
