...
вызываться через механизм удалённого вызова (RPC). Так, в примере ниже
обработка события `C` в жизненном цикле заявки на МСП-услугу осуществляется с
помощью дополнительной функции. Для этого её необходимо вызывать из приложения,
в котором работает оператор, выдающий результат оказания услуги.
...
Реализация функций обычно разнится от библиотеки к библиотеке. Сервис не
накладывает никаких ограничений на реализацию за исключением того, что
запрещено использовать посторонние решения, кроме тех, что прописаны в сервисе
(`activesupport`, `rufus-scheduler` и некоторые другие). Отметим, что эти
функции полностью покрывают жизненный цикл заявки на МСП-услугу.

## Обработка событий ручного изменения состояния

В жизненном цикле заявки выделяются события, созданные в результате ручного
...
состояния заявки на МСП-услугу.

Для их описания портируем движок обработчика подобных событий из класса
`MFCCase::Base::StateDrivenFSA` в класс `MSPCase::Base::StateDrivenFSA`. Этот
движок предлагает язык для описания переходов и действий, связанных с ними.
После портирования используем движок, создав описания переходов в файле
...
Теперь добавим реализацию обработчиков событий:
```ruby
...
Обработка сообщений СМЭВ также приводит к изменению состояния заявки. Так, к
событиям, связанными с сообщениями СМЭВ, относятся события `B1`, `B2`, `B3`
графа переходов состояния заявки на МСП-услугу. С использованием движка,
который аналогичен вышеприведённому, переходы описываются следующим образом:
```ruby
...
Тогда результирующее описание обработчиков переходов выглядит следующим
образом:
```ruby
...
Пример готовой реализации движка переходов находится в проекте `msp_case`.
Отметим следующие конструкции в языке описании переходов.
1.  Конструкция `on`, с помощью которой указывается формат обрабатываемого
    сообщения СМЭВ для данной дуги.
2.  Конструкция `if`, с помощью которой указывается условие на дугу.
