  ...
  module API
    module STOMP
      class Controller
        ...
        #
        # Класс обработчиков сообщений STOMP
        #
        class Processor
          include Helpers::Log
          ...
          # Осуществляет разбор сообщения STOMP следующим образом.
          #
          # 1.  Разбор начинается с изучения заголовка `x_message_id`. Значение
          #     в этом заголовке интерпретируется в качестве идентификатора
          #     сообщения. При отсутствии этого заголовка разбор
          #     останавливается с созданием соответствующего исключения.
          #
          # 2.  Значение заголовка `x_entities` интерпретируется в качестве
          #     названия модуля пространства имён `CaseCore::Actions` (значение
          #     предварительно приводится к форме множественного числа и
          #     ВерблюжьемуРегистру). При отсутствии этого заголовка разбор
          #     останавливается с созданием соответствующего исключения.
          #
          # 3.  Значение заголовка `x_action` интерпретируется в качестве
          #     названия функции модуля, найденного во время этапа 2 (значение
          #     предварительно приводится к змеиному_регистру). При отсутствии
          #     этого заголовка разбор останавливается с созданием
          ...
          #     время этапа 3, и аргументом, восстановленным из тела сообщения
          #     во время этапа 4.
          #
          ...
          #         `processing_statuss` со значением поля `status`, равным
          #         `error`.
          #     *   Если исключение не возникает, то создаётся запись в таблице
          #         `processing_statuss` со значением поля `status`, равным
          #         `ok`.
          #
          #     В обоих случаях в записи сохраняются заголовки сообщения вместе
          #     со значением заголовка `x_message_id` (или с `nil`, если это
          ...
          # Возвращает объект, чьё JSON-представление хранится в теле сообщения
          # STOMP
          #
          # @return [Object]
