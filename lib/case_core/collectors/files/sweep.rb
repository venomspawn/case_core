...
module CaseCore
  ...
  # Модуль пространств имён классов объектов, осуществляющих удаление ненужных
  # записей
  module Collectors
    # Пространство имён классов объектов, осуществляющих удаление записей
    # файлов, на которые не ссылается ни одна запись электронных копий
    # документов
    module Files
      ...
      class Sweep
        ...
        def invoke
          ...
          # nodoc
          delete_files unless files.empty?
          log_finish(binding)
        end
        ...
        # Запрос Sequel на извлечение идентификаторов записей файлов, на
        # которые ссылаются электронные копии документов
        ...
        # Запрос Sequel на извлечение записей файлов, на которые не ссылается
        # ни одна запись документа
        FILES_DATASET =
          Sequel::Model
          .db[:files]
        ...
        # Возвращает выражение Sequel для извлечения записей файлов, созданных
        # не позже, чем дата и время, возвращаемые {max_created_at}
        ...
        # Удаляет записи файлов
        def delete_files
          Sequel::Model.db[:files].where(id: file_ids).delete
        end

        # Сообщение о начале работы
        LOG_START = <<-LOG.squish.freeze
          Начинается проверка на наличие записей файлов, на которые не
          ссылается ни одна запись электронных копий документов, а также
        ...
        LOG_FINISH = <<-LOG.squish.freeze
          ...
          запись электронных копий документов, и удаление этих записей
          завершены
        LOG
